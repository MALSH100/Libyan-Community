...
client.on('interactionCreate', async interaction => {
  ...
  if (!interaction.guild) {
    await interaction.reply({ content: '❌ These commands only work inside a Discord server, not in DMs.', { ephemeral: true } }).catch(() => {});
    return;
  }
  ...
  try {
    ...
    await safeReply(interaction, { content: '❌ Something went wrong. Please try again.', { ephemeral: true } });
  }
});
...
async function handleCommand(interaction, commandName, user, guild) {
  ...
  if (commandName === 'clan-commands') {
    ...
    await safeReply(interaction, { embeds: [pages[0]], components: [buildRow(0)], { ephemeral: true } });
