...
// Returns only clan entries (excludes internal __ keys like __pokemon)
function getClanEntries(guildId) {
...
function getAllCommands() {
  ...
  try {
    newsCommands = initLibyaNews.commands || [];
  } catch (e) {
...
async function handleCommand(interaction, commandName, user, guild) {
    ...
    const pages = [
      ...
      new EmbedBuilder().setColor(0xE91E63).setTitle('🎴 Libyan Community Bot — Page 5/6: Collection Game (Qa\'ima)')
        ...
        .addFields(
          ...
          { name: '⚔️ How a raid works', value: ['• `/gacha-raid` lets you try to take an owned card — for a fee (50%–100% of its value by rarity)', '• Must be run in a **public channel** so the owner can see & defend', '• The owner has **24h** to react ❌ to **defend**', '• Defended → you lose the fee, the owner keeps the card + 10% of your fee', '• Not defended → the card is **yours**', '• 1 raid/day, max 3 active, and each card is safe for 2 days after a raid', '• **Active owners almost never lose** — just defend!'].join('\n') },
  ...
  if (commandName === 'libyan-stats') {
    ...
    // Get Dinar balance (collection game currency)
    const dinarBalance = db[guild.id]?.__gacha?.dinar?.[target.id] || 0;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '💰 Dinar',           value: `${dinarBalance.toLocaleString('en-US')} Dinar`, inline: true },
      )
